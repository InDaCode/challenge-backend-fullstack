...
public class BridgeService {
    ...
    public GetAccountResponse getListOfClientAccounts() throws IOException {
        var authorization = authenticateUser(USER_EMAIL, USER_PASSWORD);
        String accessToken;

        if(authorization.isPresent()){
            accessToken = authorization.get().accessToken;
        }
        else {
            accessToken = "";
        }
        return bridgeClient.getAccounts(version, "application/json", "Bearer " + accessToken, clientId, clientSecret).execute().body();

    }


    public Long getAccountsTotalBalance(List<Account> accounts){
        double sum = accounts.stream()
                .map(account -> account.balance).mapToDouble(Double::doubleValue).sum();

        long i = (long) Math.ceil(sum);
        return ((i + 99) / 100) * 100;
    }
